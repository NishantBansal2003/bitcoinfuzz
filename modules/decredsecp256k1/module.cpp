...
namespace bitcoinfuzz
{
	namespace module
	{
		...
		std::optional<std::string> Decred_secp256k1::private_to_public_key(std::span<const uint8_t> buffer) const
		{
		...
		std::optional<std::string> Decred_secp256k1::sign_compact(std::span<const uint8_t> buffer, std::span<const uint8_t> hash) const
		{
			ByteArray buffer_data{
				.data = reinterpret_cast<char *>(const_cast<uint8_t *>(buffer.data())),
				.length = static_cast<int>(buffer.size())};
		...
		std::optional<std::string> Decred_secp256k1::sign_der(std::span<const uint8_t> buffer, std::span<const uint8_t> hash) const
		{
			ByteArray buffer_data{
				.data = reinterpret_cast<char *>(const_cast<uint8_t *>(buffer.data())),
				.length = static_cast<int>(buffer.size())};
		...
		std::optional<bool> Decred_secp256k1::sign_verify(std::span<const uint8_t> buffer, std::span<const uint8_t> hash, std::span<const uint8_t> sign) const
		{
			ByteArray buffer_data{
				.data = reinterpret_cast<char *>(const_cast<uint8_t *>(buffer.data())),
				.length = static_cast<int>(buffer.size())};
		...
		std::optional<std::string> Decred_secp256k1::ecdh(std::span<const uint8_t> buffer) const
		{
			ByteArray buffer_data{
				.data = reinterpret_cast<char *>(const_cast<uint8_t *>(buffer.data())),
				.length = static_cast<int>(buffer.size())};

			char *result = DecredECDH(buffer_data);
			if (!result)
				return std::nullopt;
